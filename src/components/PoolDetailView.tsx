          ...
          {pool.placeForeignerTags && pool.placeForeignerTags.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-0.5">
              {pool.placeForeignerTags.slice(0, 5).map((tag) => {
                ...
                if (!fl) return null;
                return (
                  <span key={tag} title={fl[locale]} className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
                    <span aria-hidden="true">{fl.emoji}</span><span>{fl[locale]}</span>
                  </span>
                );
              })}
            </div>
