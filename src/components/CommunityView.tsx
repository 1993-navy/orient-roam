    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-2">
        ...
        <section>
          ...
          {isAuthed && <CreateMeetupForm cities={cities} onCreate={(b) => post("/api/meetups", b, "create")} busy={busy === "create"} />}
          ...
          <ul className="mt-4 space-y-3">
            {meetups.map((m) => {
              ...
              return (
                <li key={m.id} className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-neutral-900">
                  <div className="flex items-center gap-2 text-xs font-semibold text-rose-600">
                    <span>{label?.emoji} {label?.en ?? m.type}</span>
        ...
        <section>
          ...
          <ul className="mt-4 space-y-3">
            {communities.map((c) => (
              <li key={c.id} className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-neutral-900">
                <div className="flex items-baseline justify-between">
                  <h3 className="font-semibold">{c.name}</h3>
                  {c.cityName && <span className="text-xs text-neutral-400">{c.cityName}</span>}
                </div>
                {c.description && <p className="mt-0.5 text-sm text-neutral-500">{c.description}</p>}
                <div className="mt-2 flex items-center justify-between text-xs text-neutral-500">
